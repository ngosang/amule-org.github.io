...
- [`amulegui`](amulegui.md) — graphical remote GUI (same interface as `amule`)
...
:::note
Do not set `MaxConnections` above **1024** in [`amule.conf`](../amule-files/amule-conf.md). The wxBase library that `amuled` uses cannot handle more simultaneous connections than that.
:::
...
Alternatively, run `amuled` once normally — it will create [`~/.aMule/amule.conf`](../amule-files/amule-conf.md) with defaults — then quit and edit the file manually.

### Key [`amule.conf`](../amule-files/amule-conf.md) Settings
...
See the individual pages for [amulegui](amulegui.md), [amuleweb](amuleweb.md), and [amulecmd](amulecmd.md) for full setup instructions.
