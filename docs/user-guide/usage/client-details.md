...
The **User ID** field shows the client's numeric ID on the server it is connected to, with the ID type ([High ID or Low ID](../../ed2k/high-id-low-id.md)) shown in brackets. If the client has not yet been identified or is connected only through Kademlia, the ID is 0:
