...
| `Ed2kServersUrl` | `https://upd.emule-security.org/server.met` | URL to download `server.met` updates from. Used when auto-update is enabled. Supports `http://`, `https://`, and `ftp://`. |
...
| Key | Default | Description |
|---|---|---|
| `KadNodesUrl` | `https://upd.emule-security.org/nodes.dat` | URL to download a fresh `nodes.dat` for Kademlia bootstrap. Supports `http://`, `https://`, and `ftp://`. |

### Online Signature

| Key | Default | Description |
|---|---|---|
| `OnlineSignature` | `0` | Enable writing of `amulesig.dat` and `onlinesig.dat`. |
...
External Connections (EC) settings used by remote tools (AMuleWeb, AMuleCMD, AMuleGUI), plus several display and security preferences.
...
AMuleWeb built-in web server settings.
...
| `CoreCommand` | _(empty)_ | Shell command run by `amuled` or the monolithic `amule` binary. |
...
If you specify both a core command and a GUI command, the monolithic `amule` binary will execute both.
...
| `URL_2` | Server list — manual download | `HTTP_ServerMet` |
| `URL_3` | Server list — auto-update from `addresses.dat` | `HTTP_ServerMetAuto` |
| `URL_4` | Version check file (`last_version_check`) | `HTTP_VersionCheck` |
| `URL_5` | Kademlia nodes list (`nodes.dat`) | `HTTP_NodesDat` |
