...
weight: 10
description: >
  Primera ejecución, configuración del ancho de banda, conexión a las redes ED2K y Kademlia,
  búsqueda de archivos y gestión de descargas.
---

## Qué es aMule

aMule es un cliente P2P (peer-to-peer) para la red [eD2k](https://es.wikipedia.org/wiki/EDonkey), conocida comúnmente como red eDonkey o eD2k (eDonkey2000). Esta guía no requiere que conozcas estas redes en detalle, pero sí requiere que tengas aMule instalado en tu equipo. Si aún no lo has instalado, consulta la guía de instalación para tu plataforma.
...
> **Nota:** aMule hace un uso extensivo de los menús contextuales del botón derecho. Si no encuentras una función, prueba a hacer clic derecho sobre el elemento que quieras manipular.
...
La red eDonkey obliga a compartir para poder descargar. Para poder descargar archivos debes compartir tú también (no te preocupes si no tienes nada que compartir todavía). Esto se aplica de dos formas:

- Tu velocidad de descarga depende de tu velocidad de subida. Si la velocidad de subida está por debajo de 10 kB/s, la velocidad máxima de descarga será aproximadamente 3–4 veces la de subida. Limitar la subida a 5 kB/s significa que solo podrás descargar a ~20 kB/s.
...
Al abrir el diálogo de Preferencias por primera vez se muestra la página **General**. Para configurar el ancho de banda, haz clic en la pestaña **Conexión**:

![Diálogo de límites de ancho de banda](/docs/assets/bandwidth_limits.png)
...
![Lista de servidores vacía](/docs/assets/serverlist_empty.png)

La lista de servidores está vacía en la primera ejecución. Para poblarla, haz clic en el campo de texto que contiene la URL (p. ej. `http://gruk.org/server.met.gz`) y pulsa Intro. Aparecerá brevemente un diálogo mientras se descarga la lista.

![Lista de servidores ED2K poblada](/docs/assets/serverlist_ed2k.png)

Una vez que tengas la lista de servidores, haz clic en el botón grande **Conectar** en la parte superior izquierda de la ventana para conectarte a un servidor aleatorio. aMule contactará con los servidores y establecerá una conexión — espera hasta que informe de una conexión exitosa antes de continuar.

### Conectarse a la red Kademlia

Para conectarse a la red Kademlia (cuando está habilitada en las preferencias), pulsa el botón **Conectar** en la barra de herramientas superior. Ten en cuenta que conectarse manualmente a un servidor ED2K específico haciendo doble clic **no** te conecta a Kademlia.

Alternativamente, ve a la subpágina **Kad** de la página **Redes** y pulsa **Bootstrap desde clientes conocidos**. Si es la primera vez que usas Kad, actualiza tu archivo `nodes.dat` haciendo clic en el campo de texto de la URL y pulsando Intro. No es necesario repetir esto más adelante — aMule mantiene la lista de nodos actualizada mientras está en ejecución.

![Página de red Kademlia](/docs/assets/serverlist_kad.png)
...
- **Flechas verdes** — tienes un **ID Alto** y conectividad completa. Continúa normalmente.
- **Flechas amarillas** — tienes un **ID Bajo**. Un ID Bajo reduce considerablemente el rendimiento P2P. Deberás abrir y redirigir los puertos 4662 (TCP), 4665 (UDP) y 4672 (UDP) en tu router o cortafuegos.
...
![Diálogo de búsqueda](/docs/assets/search_dialog.png)

Para filtrar resultados por tipo, marca **Parámetros extendidos** y elige un **Tipo de archivo** en el menú desplegable (p. ej. *Imágenes de CD*). Selecciona el tipo de búsqueda:

- **Local** — consulta únicamente el servidor ED2K al que estás conectado. Rápida y suficiente en la mayoría de los casos.
...
![Resultados de búsqueda](/docs/assets/search_results.png)

Haz clic dos veces en **Fuentes** para ordenar por popularidad. Haz doble clic en un resultado (o selecciónalo y haz clic en **Descargar**) para añadirlo a la cola de descarga.
...
| Tipo | Descripción |
|---|---|
| **Local** | Consulta únicamente el servidor al que estás conectado. Rápida; suficiente en la mayoría de los casos. |
...
![Cola de transferencias](/docs/assets/transfers_queue.png)

Si la barra de progreso se vuelve de un azul intenso, muchas fuentes tienen ese archivo. Evita los archivos con segmentos en rojo — el rojo indica que ninguna fuente conocida tiene esa parte, y la descarga probablemente no podrá completarse.
...
| **Progreso** | Barra de progreso visual. Azul = fuentes disponibles (más oscuro = más fuentes); Rojo = ninguna fuente tiene esta parte; Negro = ya descargado; Amarillo = descargando actualmente. La delgada barra verde en la parte superior muestra el progreso general. |
| **Fuentes** | Formato: `<Preguntadas>[/Total] [+A4AF] [(Transfiriendo)]`. *Preguntadas* = fuentes consultadas; *Total* = todas las fuentes conocidas; *A4AF* = fuentes pedidas para otro archivo; *Transfiriendo* = fuentes que te están subiendo datos ahora mismo. |
...
| **Estado** | Estado actual de la descarga. *Esperando* significa que aMule espera a que una fuente empiece a subir. |
...
La cola de subidas aparece debajo de la cola de descargas y muestra los clientes que están descargando archivos de ti. No puedes detener las subidas manualmente.

Haz clic en el icono azul junto a la etiqueta **Subidas** para alternar entre ver los subidores activos y los clientes en cola esperando para descargarte.

### Iconos y su significado

La página de Transferencias utiliza pequeños iconos para indicar el estado de cada conexión de fuente.
...
| No disponible | Al cliente se le ha pedido otro archivo, no tiene las partes necesarias o no se puede alcanzar (ID Bajo). |
...
|---|---|
| Protocolo eMule | El cliente admite las extensiones del protocolo extendido de eMule (compartición de fuentes, etc.). |
...
| ID seguro (bueno) | El cliente ha sido identificado de forma segura. |
| ID seguro (malo) | El cliente ha sido marcado como actor malicioso. |

### Categorías para descargas

Las descargas pueden asignarse a categorías con nombre, cada una con un color único y una carpeta de guardado diferente.
...
Si tienes descargas incompletas de eMule, puedes copiar sus archivos temporales al directorio `Temp` de aMule y este las retomará.
...
![Botón recargar archivos compartidos](/docs/assets/reload_button.png)

**2. Añadir directorios compartidos en Preferencias**

Haz clic en **Preferencias → Directorios**:

![Preferencias — página de directorios](/docs/assets/prefs_directories.png)
...
Esta guía ha cubierto lo esencial para poner en marcha aMule: configurar el ancho de banda, conectarse a las redes ED2K y Kademlia, buscar y descargar archivos, y compartir tu propio contenido. Si encuentras algo que falta o no está claro, las contribuciones son bienvenidas.
